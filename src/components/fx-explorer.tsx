...
const SERIES: ReadonlyArray<{ key: SeriesChoice; label: string; hint: string }> = [
  { key: 'MID', label: 'Paralelo · punto medio', hint: 'promedio de los dos lados publicados' },
  { key: 'BUY', label: 'Paralelo · lado «buy»', hint: 'tal como lo etiqueta la fuente' },
  { key: 'SELL', label: 'Paralelo · lado «sell»', hint: 'tal como lo etiqueta la fuente' },
  { key: 'OFICIAL', label: 'Oficial', hint: 'el que publica el Banco Central' },
];
  ...
  return (
    <>
      <div className="brief">
        ...
        <p>
          Bolivia tiene <strong>dos precios para el dólar</strong>: el oficial, que fija el Banco
          Central, y el paralelo, al que efectivamente se consigue la divisa en el mercado. La
          distancia entre ambos es la <strong>brecha cambiaria</strong>, y es el indicador más
          directo de la tensión sobre las reservas y sobre el poder de compra.
        </p>
        <p>
          Lo que sigue mide <strong>cuánto y con qué violencia se mueve ese precio</strong>. Se
          trabaja sobre <strong>retornos logarítmicos diarios</strong> —la variación porcentual de un
          día al siguiente— porque el nivel en bolivianos no dice si un salto es grande: pasar de
          9,60 a 9,70 y de 11,60 a 11,70 son la misma cifra y no el mismo hecho. Sobre esos retornos
          se calculan la volatilidad, la forma de la distribución y el riesgo de cola.
        </p>
        <p className="brief-note">
          Los controles de abajo recalculan todo lo que aparece después: el periodo, la serie sobre
          la que se mide y la ventana móvil. Ninguna cifra de esta página está escrita a mano.
        </p>
      </div>

      <div className="slicers">
        <div className="slicer">
          <span className="slicer-label">Periodo</span>
          <div className="chips">
            {RANGES.map((entry) => (
              <button
                key={entry.key}
        ...
        <div className="slicer">
          <span className="slicer-label">Serie medida</span>
          <div className="chips">
            {SERIES.map((entry) => (
              <button
                key={entry.key}
                type="button"
                title={entry.hint}
                className={series === entry.key ? 'chip chip-on' : 'chip'}
        ...
        <div className="slicer-row">
          <div className="slicer">
            <span className="slicer-label">Ventana móvil</span>
            <div className="chips">
              {WINDOWS.map((days) => (
                <button
                  key={days}
          ...
          <div className="slicer slicer-grow">
            <span className="slicer-label">Base de medición</span>
            <div className="chips">
              <button
                type="button"
                className={pointInTimeOnly ? 'chip' : 'chip chip-on'}
        ...
        <div className="slicer-foot">
          <span>
            {chosen?.label} · {selected.length.toLocaleString('es-BO')} jornadas
            {first && last ? ` · ${first.date} a ${last.date}` : ''}
            {move === null ? '' : ` · ${signed(move)} en el periodo`}
          </span>
      ...
      {returns.length < 5 ? (
        <div className="callout">
          La selección deja menos de cinco jornadas encadenadas: no alcanza para estimar nada. Amplía
          el periodo o vuelve a «Toda la serie».
        </div>
      ) : (
        <>
          <div className="panel">
            <div className="panel-head">
              ...
              <p className="panel-sub">
                ...
                Azul: tipo de cambio oficial. El eje no arranca en cero, porque un movimiento de dos
                bolivianos es enorme y una base en cero lo aplanaría.
              </p>
            </div>
          ...
          <div className="panel">
            <div className="panel-head">
              ...
              <p className="panel-sub">
                Variación logarítmica de {chosen?.label.toLocaleLowerCase('es')}. Las barras hacen
                visibles los saltos que una línea de nivel suaviza.
              </p>
            </div>
            <SeriesChart
              data={returnSeries}
          ...
          <div className="grid-two">
            ...
            <div className="panel">
              <div className="panel-head">
                ...
                <p className="panel-sub">
                  Días por tramo; en rojo, la cola inferior del 5 %. Si las barras extremas son más
                  altas de lo que sería normal, los días excepcionales no son tan excepcionales.
                </p>
              </div>
          ...
          <div className="grid-two">
            ...
            <div className="panel">
              <div className="panel-head">
                ...
                <p className="panel-sub">
                  Distancia respecto al mayor nivel alcanzado hasta cada fecha. En un tipo de cambio
                  que se deprecia, volver a cero significa un nuevo récord.
                </p>
              </div>
